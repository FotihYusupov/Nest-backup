...
export class BackupService {
  ...
  backupDatabase(): void {
    ...
    exec(command, (error) => {
      ...
      this.zipAndSend(DATABASE_NAME, BACKUP_PATH, CHAT_ID, TOKEN);
    });
  }

  async zipAndSend(DATABASE_NAME: string, BACKUP_PATH: string, CHAT_ID: string, TOKEN: string): Promise<void> {
    ...
    output.on('close', () => {
      ...
      this.sendDocumentToTelegramChannel(`./backup/${DATABASE_NAME}.zip`, CHAT_ID, TOKEN)
        .then(() => {
  ...
  async sendDocumentToTelegramChannel(
    filePath: string,
    chatId: string,
    botToken: string
  ): Promise<void> {
